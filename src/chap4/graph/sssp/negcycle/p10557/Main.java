package chap4.graph.sssp.negcycle.p10557;
...
public class Main {
	...
	static int process() throws Exception{
	  ...
	  if(cnt == -1)
	    return 0;
	  List<IntegerTriple> edges = new ArrayList<Main.IntegerTriple>();
	  int[] en = new int[cnt];
	  for(int i = 0; i < cnt; i++) {
	    int a = in.readInt();
	    en[i] = a;
	    int b = in.readInt();
	    for(int j = 0; j < b; j++) {
	      edges.add(new IntegerTriple(i, in.readInt() - 1, 0));
	    }
	  }
	  ...
	  boolean[] reach = new boolean[cnt];
	  reach[cnt - 1] = true;
	  int add = 1;
	  while(add != 0) {
	    add = 0;
	    for(IntegerTriple it : edges) {
	      if(reach[it.b] && !reach[it.a]) {
	        reach[it.a] = true;
	        add++;
	      }
	    }
	  ...
	  for(int i = 1; i < cnt; i++) {
	    for(IntegerTriple it : edges) {
	      if(vis[it.a]) {
	        long dis = st[it.a] + it.c;
	        if(dis > st[it.b] && vis[it.a] && st[it.a] > 0) {
	          vis[it.b] = true;
	          st[it.b] = dis;
	        }
	      }
	  ...
	  for(IntegerTriple it : edges) {
	    long dis = st[it.a] + it.c;
	    if(dis > st[it.b] && vis[it.a] && st[it.a] > 0) {
	      st[it.b] = Integer.MAX_VALUE;
          if(reach[it.b]) 
            st[cnt - 1] = Integer.MAX_VALUE;
	    }
	  }
	  //System.out.println(Arrays.toString(st));
	  System.out.println(st[cnt-1] >= 0 ? "winnable" : "hopeless");
	  return 1;
	}
