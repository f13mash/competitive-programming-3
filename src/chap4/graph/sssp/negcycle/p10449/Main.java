package chap4.graph.sssp.negcycle.p10449;
...
public class Main {
	...
	static int process() throws Exception{
	  int cnt = in.readInt();
	  int[] busy = cnt > 0 ? IOUtils.readIntArray(in, cnt) : new int[0];
	  int rc = in.readInt();
	  //System.out.println(Arrays.toString(busy));
	  List<Main.IntegerTriple> ll = new ArrayList<Main.IntegerTriple>();
	  for(int i = 0; i < rc; i++) {
	    int a = in.readInt() - 1, b = in.readInt() - 1, c = busy[b] - busy[a];
	    ll.add(new IntegerTriple(a, b, c*c*c));
	  }
	  long[] dis = new long[cnt];
	  boolean[] vis = new boolean[cnt];
	  Arrays.fill(dis, Integer.MAX_VALUE);
	  if(cnt > 0) {dis[0] = 0; vis[0] = true;}
	  for(int i = 1; i < cnt; i++) {
	    for(IntegerTriple it : ll) {
	      if(dis[it.b] > (dis[it.a] + it.c) && vis[it.a]) {
	  ...
	  boolean brk = false;
        for(IntegerTriple it : ll) {
          if(dis[it.b] > (dis[it.a] + it.c) || !vis[it.b]) {
            dis[it.b] = Integer.MIN_VALUE;
          }
        }
	  int q = in.readInt();
	  System.out.println(String.format("Set #%d", casen));
	  for(int i = 1; i <= q; i++) {
	    int a = in.readInt() - 1;
	    //System.out.println(i+" : "+a+" : "+q);
	    if(dis[a] < 3 || dis[a] == Integer.MAX_VALUE)
	      System.out.println("?");
	    else
