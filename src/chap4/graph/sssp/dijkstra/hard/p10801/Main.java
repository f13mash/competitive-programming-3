package chap4.graph.sssp.dijkstra.hard.p10801;
...
public class Main {
	...
	static int process() throws Exception{
		...
		int[] cost = giveArray(readLine().trim().split(" "));
		int[] res = new int[101];
		Arrays.fill(res, -1);
		res[0] = 60;
		...
		for(int i = 0; i < n; i++) {
			...
			int[] flrs = giveArray(readLine().trim().split(" "));
			for(int a : flrs) {
				for(int b : flrs) {
					int c = Math.abs(a-b) * cost[i];
					mp[a][b] = (mp[a][b] == -1 ?  c : Math.min(mp[a][b], c));
				}
			}
		}
		//System.out.println(Arrays.toString(cost));
		...
		PriorityQueue<IntegerPair> pq = new PriorityQueue<IntegerPair>(n * 101, new Comparator<IntegerPair>() {
			@Override
		...
		while(pq.size() > 0) {
			...
			for(int i = 0; i < mp[ip.a].length; i++) {
				if(mp[ip.a][i] != -1) {
					int c = ip.b + mp[ip.a][i] + 60;
					if(res[i] == -1 || res[i] > c) {
						res[i] = c;
						pq.add(new IntegerPair(i, c));
					}
				}
			}
