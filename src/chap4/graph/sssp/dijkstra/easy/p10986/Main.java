package chap4.graph.sssp.dijkstra.easy.p10986;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			int n = in.readInt(), m = in.readInt(), s = in.readInt(), t = in.readInt();
			...
			for(int i = 0; i < m; i++) {
				...
				mp.get(a).add(new IntegerPair(b, c));
