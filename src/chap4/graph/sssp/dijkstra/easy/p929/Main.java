package chap4.graph.sssp.dijkstra.easy.p929;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			PriorityQueue<IntegerTriple> pq = new PriorityQueue<>(r*c, new Comparator<IntegerTriple>() {
				@Override
				public int compare(IntegerTriple o1, IntegerTriple o2) {
					return o1.c - o2.c;
				}
			});
			pq.add(new IntegerTriple(0, 0, res[0][0]));
			...
			while(pq.size() > 0) {
				...
				for(int i = 0; i < dirs.length; i++) {
					int na = it.a + dirs[i][0];
					int nb = it.b + dirs[i][1];
					if(na >= 0 && na < r && nb >= 0 && nb < c && (res[na][nb] == -1 || res[na][nb] > (it.c + mp[na][nb]))) {
						res[na][nb] = it.c + mp[na][nb];
						pq.add(new IntegerTriple(na, nb, res[na][nb]));
					}
				}
			}
