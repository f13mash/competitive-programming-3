package chap4.graph.sssp.bfshard.p12160;
...
public class Main {
	...
	static int process() throws Exception{
		int l = in.readInt(), u = in.readInt(), r = in.readInt();
		if(l == 0 && u == 0 && r == 0)
			return 0;
		...
		vis[l] = 0;
		PriorityQueue<IntegerPair> pq = new PriorityQueue<>(10010, new Comparator<IntegerPair>() {
			@Override
		...
		pq.add(new IntegerPair(l, 0));
		while(pq.size() > 0 && vis[u] == -1) {
			IntegerPair ip = pq.remove();
			for(int i = 0; i < add.length && vis[u] == -1; i++) {
				int a = (ip.a + add[i]) % 10000;
				if(vis[a] != -1)
					continue;
				vis[a] = ip.b + 1;
				pq.add(new IntegerPair(a, vis[a]));
			}
		}
		
		String str = "Permanently Locked";
		if(vis[u] != -1) {
			str = "" + vis[u];
		}
		System.out.println(String.format("Case %d: %s", casen++, str));
