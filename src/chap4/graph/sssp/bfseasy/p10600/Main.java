package chap4.graph.sssp.bfseasy.p10600;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			String st = readLine().trim();
			
			List<char[]> words = new ArrayList<>();
			while(st.compareTo("*") != 0) {
				words.add(st.toCharArray());
				st = readLine().trim();
			}
			
			st = readLine();
			while(st != null && st.trim().length() != 0) {
				String[] splt = st.trim().split(" ");
				char[] ca = splt[0].trim().toCharArray(), cb = splt[1].trim().toCharArray();
				int sz = ca.length;
				
				Set<char[]> space = new HashSet<>();
				for(char[] tm : words) {
					if(tm.length == sz)
						space.add(tm);
				}
				...
				LinkedList<char[]> state = new LinkedList<>();
				state.add(ca);
				space.remove(ca);
				...
				System.out.println(new String(ca)+" "+new String(cb)+" "+changes);
				st = readLine();
