package chap4.graph.toposort.p11060;
...
public class Main {
	...
	static int process() throws Exception{
		...
		int m = in.readInt();
		for(int i = 0; i < m; i++) {
			int a = mp.get(in.readString());
			int b = mp.get(in.readString());
			rel[b].set(a);
		}
		...
		for(int i = 0; i < n; i++) {
			if(!vis[i])
				topoSort(rel, i, vis, res);
		}
		sb.append(String.format("Case #%d: Dilbert should drink beverages in this order:", casen++));
	...
	static void topoSort(BitSet[] rel, int curr, boolean[] vis, List<Integer> res) {
		int n = rel.length;
		while(n != res.size()) {
			for(int i = 0; i < vis.length; i++) {
				if(vis[i] || rel[i].cardinality() > 0) 
					continue;
				res.add(i);
				vis[i] = true;
