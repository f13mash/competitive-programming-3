package chap4.graph.floodfill.p11094;
...
public class Main {
	...
	static int process() throws Exception{
		...
		int ci = in.readInt() + 1, cj = in.readInt();
		char c = mp[ci][cj];
		ffill(mp, ci, cj, c);
		int bst = 0;
		for(int i = 1; i <= m; i++) {
			for(int j = 0; j < n; j++) {
				if(mp[i][j] == c) {
					bst = Math.max(ffill(mp, i, j, c), bst);
				}
			}
		}
	...
	static int ffill(char[][] mp, int i, int j, char c) {
		int cnt = 0;
		...
		while(li.size() > 0) {
			...
			if(mp[ci][cj] == c) {
				...
				for(int a = 0; a < dirs.length; a++) {
					int ni = ci + dirs[a][0], nj = cj + dirs[a][1];
					nj+=(mp[0].length);
					nj%=(mp[0].length);
					if(mp[ni][nj] != c)
						continue;
