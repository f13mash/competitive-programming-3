package chap4.graph.mst.std.p11228;
...
public class Main {
		...
		while(cases-- > 0) {
			...
			PriorityQueue<Edge> pq = new PriorityQueue<>(n > 1 ? n*(n-1) : 1, new Comparator<Edge>() {
				...
				public int compare(Edge o1, Edge o2) {
					return o1.distanceSq() - o2.distanceSq();
				}
			});
			...
			while(pq.size() > 0) {
				...
				uf.add(e.a, e.b);
				if(dis > r)
					railCost += Math.sqrt(dis);
				else
					roadCost += Math.sqrt(dis);
			}

			System.out.printf("Case #%d: %d %d %d\n", casen, states, (int) Math.round(roadCost), (int) Math.round(railCost));
			casen++;
		}
		return 0;
	}
	...
	static class Edge { 
		...
		public int distanceSq() { 
			return (int) points[a].distanceSq(points[b]);
		}
	}
