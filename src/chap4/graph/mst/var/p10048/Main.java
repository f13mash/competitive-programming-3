package chap4.graph.mst.var.p10048;
...
public class Main {
	...
	static int process() throws Exception{
		int m = in.readInt(), n = in.readInt(), q = in.readInt();
		if(m == 0 && n == 0 && q == 0)
			return 0;
	...
	static int dfs(Map<Integer, List<Edge>> mp, int curr, int last, int dest) {
		for(Edge e : mp.get(curr)) {
			if(e.other(curr) == last)
				continue;
			if(e.other(curr) == dest) {
				return e.w;
			}
			int res = dfs(mp, e.other(curr), curr, dest);
			if(res == -1)
				continue;
			return Math.max(res, e.w);
		}
