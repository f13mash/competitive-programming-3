package chap4.graph.apsp.var.p104;
...
public class Main {
	...
	static int process() throws Exception{
	  int sz = in.readInt();
	  double[][][] cost = new double[sz+1][sz][sz];
      int[][][] path = new int[sz+1][sz][sz];
	  ...
	  for(int i = 0; i < sz; i++) {
	    for (int j = 0; j < sz; j++) {
	      if(i == j) {
	        ...
	        cost[1][i][j] = Double.parseDouble(in.readString());
	      }
	  ...
	  for(int a = 2; a <= sz; a++) {
	    for(int k = 0; k < sz; k++) {
	      for(int i = 0; i < sz; i++) {
	        for(int j = 0; j < sz; j++) {
              double cst = cost[a-1][i][k] * cost[1][k][j];
              if(cst > cost[a][i][j]) {
                cost[a][i][j] = cst;
                path[a][i][j] = k;
              }
	  ...
	  boolean found = false;
      String str = "no arbitrage sequence exists";

	  for(int a = 1; a <= sz && !found; a++) {
	    for(int i = 0; i < sz; i++) {
	      if(cost[a][i][i] > 1.01) {
	        found = true;
