package chap4.graph.apsp.std.p11463;
	...
	static int process() throws Exception{
	  ...
	  while(cases-- > 0) {
	    ...
	    for(int i = 0; i < sz; i++) {
	      for (int j = 0; j < sz; j++) {
	        for (int k = 0; k < sz; k++) {
	          mp[j][k] = Math.min(mp[j][k], mp[j][i] + mp[i][k]);
              mp[k][j] = Math.min(mp[k][j], mp[j][i] + mp[i][k]);

	        }
          }
	    }
	    
	    //System.out.println("size "+mp.length);
	    
	    int s = in.readInt(), e = in.readInt();
	    int ans = 0;
	    for(int i = 0; i < sz; i++) {
	      ans = (int) Math.max(ans, mp[s][i] + mp[i][e]);
	    }
