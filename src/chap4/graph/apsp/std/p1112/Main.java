package chap4.graph.apsp.std.p1112;
...
public class Main {
	...
	static int process() throws Exception{
	  ...
	  if(sz == 0)
	    return 0;
	  Map<Character, Integer> idx = new HashMap<Character, Integer>();
	  long[][][] mp = new long[2][26][26];
	  for(int i = 0; i < 26; i++) {
	    Arrays.fill(mp[0][i], Integer.MAX_VALUE);
        Arrays.fill(mp[1][i], Integer.MAX_VALUE);
	  }
	  for(int i = 0; i < sz; i++) {
	    boolean youth = in.readString().charAt(0) == 'Y';
	    boolean unid = in.readString().charAt(0) == 'U';
	    char ca = in.readString().charAt(0);
	    char cb = in.readString().charAt(0);
	    int cost = in.readInt();
	    if(!idx.containsKey(ca)) {
	      idx.put(ca, idx.size());
	    }
	    if(!idx.containsKey(cb)) {
          idx.put(cb, idx.size());
        }
	    int a = idx.get(ca);
	    int b = idx.get(cb);
	    int ind = youth ? 0 : 1;
	    mp[ind][a][b] = Math.min(mp[ind][a][b], cost);
	    if(!unid)
	      mp[ind][b][a] = Math.min(mp[ind][b][a], cost);
	  }
	  char caa = in.readString().trim().charAt(0);
	  char cbb = in.readString().trim().charAt(0);
	     if(!idx.containsKey(caa)) {
	          idx.put(caa, idx.size());
	        }
	     if(!idx.containsKey(cbb)) {
           idx.put(cbb, idx.size());
         }

	  int posA = idx.get(caa);
	  int posB = idx.get(cbb);
	  int l = idx.size();
	  for(int i = 0; i < 26; i++)
	    mp[0][i][i] = mp[1][i][i] = 0;
	  for(int i  = 0; i < l; i++) {
	    for(int j = 0; j < l; j++) {
	      for(int k = 0; k < l; k++) {
	        mp[0][j][k] = Math.min(mp[0][j][k], mp[0][j][i] + mp[0][i][k]);
	      }
	    }
	  }
	  for(int i  = 0; i < l; i++) {
        for(int j = 0; j < l; j++) {
          for(int k = 0; k < l; k++) {
            mp[1][j][k] = Math.min(mp[1][j][k], mp[1][j][i] + mp[1][i][k]);
          }
        }
      }
	  ...
	  for(char i = 'A'; i <= 'Z'; i++) {
	    if(!idx.containsKey(i))
	      continue;
	    int id = idx.get(i);
	    if(mp[0][posA][id] >= Integer.MAX_VALUE || mp[1][posB][id] >= Integer.MAX_VALUE) {
	      continue;
	    }
	    if((mp[0][posA][id] + mp[1][posB][id]) == cost) {
          places.add(i);
          cost = mp[0][posA][id] + mp[1][posB][id]; 
        }
	    if((mp[0][posA][id] + mp[1][posB][id]) < cost) {
	      places.clear();
	      places.add(i);
	  ...
	  if(places.size() == 0) {
	    ...
	    Set<Character> cl = new HashSet<Character>();
	    cl.addAll(places);
	    places.clear();
	    places.addAll(cl);
