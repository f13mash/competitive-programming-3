package chap4.graph.traversal.p11831;
		...
		for(int i = 1; i <= n; i++) {
			...
			for(int j = 1; j <= m; j++) {
				...
				case '#': 
					...
					break;
				case 'N':
					si = i; sj = j; dir = 0; break;
				case 'S':
					si = i; sj = j; dir = 2; break;
				case 'L':
					si = i; sj = j; dir = 1; break;
				case 'O':
					si = i; sj = j; dir = 3; break;
				default:
					break;
				}
			}
		}
		char[] cmd = s < 0 ? null : in.readString().toCharArray();
		int cnt = 0;
		for(int i = 0; i < s; i++) {
			char c = cmd[i];
			switch (c) {
			...
			case 'E':
				...
				break;
			case 'F':
				if(mp[si+dirs[dir][0]][sj+dirs[dir][1]] != -1) {
					si += dirs[dir][0];
					sj += dirs[dir][1];
					if(mp[si][sj] > 0) {
						mp[si][sj] = 0;
