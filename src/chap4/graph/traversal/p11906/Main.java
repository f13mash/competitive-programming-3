package chap4.graph.traversal.p11906;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			int r = in.readInt(), c = in.readInt(), n = in.readInt(), m = in.readInt();
			len = -1; e = 0; o = 0;
			int[][] vis = new int[r][c];
			...
			int[][] dirs = {{-n,-m}, {-n, m}, {n, -m}, {n, m}, {-m, -n}, {-m, n}, {m, -n}, {m, n}};
			recur(vis, 0, 0, dirs);
			int odd = 0, even = 0;
			for(int i = 0; i < r; i++) {
				for(int j = 0; j < c; j++) {
	...
	static int len = -1, e = 0, o = 0;
	static void recur (int[][] vis, int ci, int cj, int[][] dirs) {
		//System.out.println(ci+" : "+cj+" : "+vis[ci][cj]);
		int r = vis.length, c = vis[0].length;
		if(vis[ci][cj] > 0)
			return;
