package chap4.graph.stronglyconnected.p11504;
...
public class Main {
	...
	static int process() {
		...
		int lim = 100002;
		int[] st = new int[lim + 1];
		boolean[] vis = new boolean[lim];
		int[] visid = new int[lim];

		Set<Integer>[] arr = new HashSet[lim];
		for(int i = 0; i < lim; i++)
			arr[i] = new HashSet<>();
...
//					System.out.println(i+" : "+st[0]);
					//dfs(arr, dfs_num, dfs_low, i, st, ine, vis);
					dfs2(arr, dfs_num, dfs_low, i, st, ine, visid);
				}
			}
			for(int i = 0; i < n; i++) {
				arr[i].clear();
	...
	// a non-recursive dfs version using Tarjan. Also maintain number of in-edges to a strongly connected component. If it is zero, only then one of the dominoes has to be knocked down
	static void dfs2(Set<Integer>[] arr, int[] dfs_num, int[] dfs_low, int start, int[] st, int[] ine, int[] visid) {
