package chap3.divide_conquer.binary_search.p10567;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(k-- > 0) {
			...
			for(char c : inp) {
				List<Integer> al = mp.get(c);
				//System.out.println(c+" : "+mp.get(c)+" : "+ind);
				ind = findClosest(al, ind + 1, false);
				if(ind == -1) {
					break;
				}
				ind = al.get(ind);
			...
			if(ind == -1) {
				...
				System.out.println(String.format("Matched %d %d", s, ind));
			}
		}
		return 0;
	}
