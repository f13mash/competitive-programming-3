package chap3.dp.coinchange.p10306;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			for(int i = 0; i < m; i++) {
				coins[i][0] = in.readInt();
				coins[i][1] = in.readInt();
			}
			int ans = Integer.MAX_VALUE;
			for(int i = 0; i <= s; i++) {
				int y = (int) Math.sqrt((s*s) - (i*i));
				if(((y*y) + (i*i)) == (s*s)) {
					int res = recur(dp, coins, 0, i, y);
					if(res >= 0) {
						ans = Math.min(ans, res);
					}
					//System.out.println(i+" : "+y+" : "+res);
				}
			}
