package chap3.dp.coinchange.p11517;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			int ans = Integer.MAX_VALUE, x = Integer.MAX_VALUE;
			for(int i = 0; i < n; i++) {
				for(int j = mx - 1; j >= bills[i]; j--) {
					if(dp[j - bills[i]] == Integer.MAX_VALUE) {
						continue;
					}
					dp[j] = Math.min(dp[j], dp[j-bills[i]] + 1);
					if(j >= s && dp[j] != Integer.MAX_VALUE) {
						if(j < x) {
							ans = dp[j];
							x = j;
						}
	...
	static int recur(int[][] dp, int[][] coins, int curr, int x, int y) {
		if(x < 0 || y < 0)
			return -1;
		if(x == 0 && y == 0)
			return 0;
		...
		while(i >= 0 && j >= 0 && !(coins[curr][0] == 0 && coins[curr][1] == 0)) {
			int res = recur(dp, coins, curr + 1, i, j);
			if(res >= 0) {
				if(dp[x][y] == -2)
					dp[x][y] = cnt + res;
				dp[x][y] = Math.min(dp[x][y], cnt + res);
			}
			i -= coins[curr][0];
			j -= coins[curr][1];
			cnt++;
