package chap3.dp.coinchange.p357;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(true) {
			...
			long res = dp[q];
			if(res > 1) {
				sb.append(String.format("There are %d ways to produce %d cents change.\n", res, q));
			}
			else {
				sb.append(String.format("There is only 1 way to produce %d cents change.\n",q));
			}
		}
	}
	/*static int recur(int[] arr, int[][] dp, int curr, int k) {
		if(curr < 0) 
			return -1;
		if(curr == 0)
			return 1;
		...
		int cnt = 0;
		int res = recur(arr, dp, curr - (arr[k] * (cnt++)), k + 1);
		while(res >= 0) {
			dp[curr][k] += res;
			res = recur(arr, dp, curr - (arr[k] * (cnt++)), k + 1);
		}
