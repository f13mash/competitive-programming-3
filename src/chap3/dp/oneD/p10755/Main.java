...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			long[][][] mp = new long[a+2][b+2][c+2];
			long[][][] sum = new long[a+2][b+2][c+2];
			for(int i = 1; i <= a; i++) {
				for(int j = 1; j <= b; j++) {
					for(int k = 1; k <= c; k++) {
						mp[i][j][k] = Long.parseLong(in.readString());
					}
				}
			}
			for(int i = a; i >= 1; i--) {
			...
			for(int y1 = 1; y1 <= b; y1++) {
				for(int z1 = 1; z1 <= c; z1++) {
					for(int y2 = y1 + 1; y2 <= (b+1); y2++) {
						for(int z2 = z1 + 1; z2 <= (c+1); z2++) {
							long s = 0;
							for(int x = 1; x <= a; x++) {
								s += sum[x][y1][z1] - sum[x][y1][z2] - sum[x][y2][z1] + sum[x][y2][z2];
								max = Math.max(max, s);
								if(s < 0) {
									s = 0;
								}
