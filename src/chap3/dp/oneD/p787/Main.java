...
public class Main {
	...
	static int process() throws Exception{
		BigInteger prod = BigInteger.ONE, mostNeg = BigInteger.ZERO, max = new BigInteger(Long.toString(Long.MIN_VALUE));
		while(true) {
			int val = in.readInt();
			BigInteger bval = new BigInteger(Integer.toString(val));
			if(val == -999999)
				break;
			if(max.compareTo(bval) < 0) {
				max = bval;
			}
			if(val == 0) {
				prod = BigInteger.ONE;
				mostNeg = BigInteger.ZERO;
			}
			else {
				prod = prod.multiply(bval);
				if(prod.compareTo(BigInteger.ZERO) < 0) {
					if(mostNeg.compareTo(BigInteger.ZERO) == 0) {
						mostNeg = prod;
					}
					else {
						bval = prod.divide(mostNeg);
						if(max.compareTo(bval) < 0) {
							max = bval;
						}
					}
