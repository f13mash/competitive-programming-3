		...
		while(cases-- > 0) {
			...
			for(int l = 1; l <= n; l++) {
				...
				for(int r = l; f1 || r != l; r++) {
					...
					int[] val = new int[n+2];
					int min = Integer.MAX_VALUE, fmin = Integer.MAX_VALUE;
					int total = 0, sum = 0;
					for(int i = 1; i <= n; i++) {
						if(r < l) {
							val[i] = (arr[i][r] + arr[i][n] - arr[i][l-1]);
							//System.out.println(sum+" |1| "+lind+" : "+i+" ::: "+l+" : "+r+" :::: "+Arrays.toString(arr[i]));
						}
						else {
							val[i] = (arr[i][r] - arr[i][l-1]);
							//System.out.println(sum+" |2| "+lind+" : "+i+" ::: "+l+" : "+r);
						}
						if(min > 0) {
							min = val[i];
						}
					...
					//this check is not required for AC.
					if(total != fmin)
						max = Math.max(max, total - fmin);
				}
			}
