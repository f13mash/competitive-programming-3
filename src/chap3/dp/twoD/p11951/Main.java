...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			int[][] mp = new int[n+2][m+2];
			
			for(int i = 1; i <= n; i++) {
				for(int j = 1; j <= m; j++) {
					mp[i][j] = in.readInt() + mp[i][j-1];
				}
			}
			int best = Integer.MAX_VALUE, area = 0;
			for(int l = 1; l <= m; l++) {
				for(int r = l; r <= m; r++) {
					int sum = 0;
					int ind = 0;
			...
			System.out.println(String.format("Case #%d: %d %d", casen, area, best));
			casen++;
		}
		return 0;
	}
