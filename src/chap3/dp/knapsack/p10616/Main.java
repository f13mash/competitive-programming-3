package chap3.dp.knapsack.p10616;
...
public class Main {
	...
	static int process() throws Exception{
		...
		for(int qid = 1; qid <= q; qid++) {
			int d = in.readInt(), m = in.readInt();
			int[][][] dp = new int[n][m+1][d];

			for(int i = 0; i < dp.length; i++) for(int j = 0; j < dp[i].length; j++) Arrays.fill(dp[i][j], -1);
			
			sb.append(String.format("QUERY %d: %d\n", qid, eval(dp, arr, d, m, 0, 0)));
		}
		return 1;
	}
	
	static int eval(int[][][] dp, long[] arr, int d, int m, int curr, long sum) {
		//System.out.println(m+" : "+curr+" : "+sum);
		if(m == 0) {
			if((sum % d) == 0) {
				return 1;
			}
		...
		if(curr >= arr.length) 
			return 0;
		if(dp[curr][m][(int) (sum %d)] >= 0)
			return dp[curr][m][(int) (sum%d)];
		
		dp[curr][m][(int) (sum %d)] = eval(dp, arr, d, m, curr + 1, ((sum % d) + d)) + eval(dp, arr, d, m - 1, curr + 1, (sum + (arr[curr]%d) + d)%d);
