...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			for(int i = 0; i < n; i++) {
				for(int j = 0; j < i; j++) {
					dis[i][j] = dis[j][i] = (int) (Math.abs(mp[i][0] - mp[j][0]) + Math.abs(mp[i][1] - mp[j][1])) ;
				}
			}

			int[][] dp = new int[n][1 << n];
			for(int i = 0; i < dp.length; i++) {Arrays.fill(dp[i], -1); dp[i][0] = 0;}
					...
					if(i == 0) {
						...
						for(int k = 0; k < n; k++) {
							if((i & (1 << k)) > 0) {
								if(dp[k][i] == -1)
									continue;
								if((dp[k][i] + dis[k][j]) < res)
									res = dp[k][i] + dis[k][j];
							}
						}
						dp[j][ni] = res;
						if(ni == ((1 << n) - 1) && j == (n-1) && res != -1) {
							ans = Math.min(ans, dp[j][ni]);
						}
					}
				}
			}
