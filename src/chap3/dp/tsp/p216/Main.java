...
public class Main {
	...
	static int process() throws Exception{
		...
		for(int i = 0; i < (1 << n); i++) {
			for(int j = 0; j < n; j++) {
				...
				if(i == 0) {
					...
					for(int k = 0; k < n; k++) {
						if((i & (1 << k)) > 0 && dp[j][ni] > (dp[k][i] + arr[j].distance(arr[k]))) {
							dp[j][ni] = dp[k][i] + arr[j].distance(arr[k]);
							prev[j][ni] = k;
						}
					}
				}
		...
		System.out.printf("**********************************************************\nNetwork #%d\n", casen);
		for(int i = 1; i < n; i++) {
			Point2D a = arr[ans[i-1]], b = arr[ans[i]];
			System.out.printf("Cable requirement to connect (%d,%d) to (%d,%d) is %.2f feet.\n", (int)a.getX(), (int)a.getY(), (int)b.getX(), (int)b.getY(), a.distance(b) + 16.00 + 1e-9);
		}
