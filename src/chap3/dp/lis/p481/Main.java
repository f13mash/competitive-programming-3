...
public class Main {
	...
	static int process() throws Exception{
		...
		int max = 0, ind = -1, curr = 0;
		List<Long> ans = null;
		int mx = -1;
		while(ln != null && ln.trim().length() > 0) {
			long val = Long.parseLong(ln.trim());
			num.add(val);
			if(state.size() == 0) {
				ind = state.size();
				state.add(val);
				...
				if(state.get(state.size()-1) < val) {
					...
					if(state.get(state.size()-1) > val) {
						ind = findClosest(state, val, false);
						state.set(ind, val);
						indx.set(ind, curr);
					}
					else {
			...
			if(ind > 0) {
				hist.add(indx.get(ind-1));
			}
			else {
				hist.add(-1);
			}
			if(ind == (state.size()-1) && (ans == null || ans.size() <= state.size())) {
	...
	static int findClosest(List<Long> arr, long val, boolean comp, int s, int e) {
	...
	static int findClosest(List<Long> arr, long val, boolean comp) {
