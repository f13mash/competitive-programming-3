	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			for(int i = 1; i <= n; i++) {
				...
				for(int j = i + 1; j <= n; j++) {
					if(arr[j] > v) {
						int ind = findClosest(incl, arr[j], false, 1, incl[0]);
						if(ind == -1) {
							incl[0]++;
							incl[incl[0]] = arr[j];
						}
				...
				for(int j = i + 1; j <= n; j++) {
					if(arr[j] < v) {
						int ind = findClosest(decl, arr[j], true, n + 1 - decl[n+1], n);
						if(ind == -1) {
							decl[n+1]++;
							decl[n+1-decl[n+1]] = arr[j];
						}
						else {
							decl[ind] = arr[j];
						}
				...
				res = Math.max(res, incl[0] + decl[n+1] - 1);
			}
			sb.append(res+"\n");
