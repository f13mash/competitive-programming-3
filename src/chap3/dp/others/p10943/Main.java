package chap3.dp.others.p10943;
...
public class Main {
	...
	static int process() throws Exception{
		...
		List<IntegerPair> q = new ArrayList<>();
		while(a != 0 || b != 0) {
			q.add(new IntegerPair(a, b));
			//n = Math.max(a, n);
			//k = Math.max(b, k);
			a = in.readInt();
			b = in.readInt();
		...
		for(int i = 2; i < k; i++) {
			for(int j = 0; j < n; j++) {
				for(int add = 0; add < n; add++) {
					if((j + add) < n) {
						res[i][j+add] += res[i-1][j];
						res[i][j+add] %= 1000000;
					}
				}
			}
		}
		for(IntegerPair ip : q) {
			sb.append(res[ip.b][ip.a]+"\n");
		}
		return 0;
	}
