package chap3.dp.others.p10337;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			int n = in.readInt()/100;
			int[][] mp = new int[10][];
			int[][] dp = new int[n+1][10];

			for(int i = 0; i < 10; i++) {
				mp[9-i] = IOUtils.readIntArray(in, n);
			}
			...
			for(int i = 0; i < n; i++) {
				for(int j = 0; j < 10; j++) {
					...
					if((j-1) >= 0) {
						if(dp[i+1][j-1] == -1 || dp[i+1][j-1] > (dp[i][j] + s - mp[j][i])) {
							dp[i+1][j-1] = dp[i][j] + s - mp[j][i];
						}
					}
					if((j+1) < 10) {
						if(dp[i+1][j+1] == -1 || dp[i+1][j+1] > (dp[i][j] + r - mp[j][i])) {
							dp[i+1][j+1] = dp[i][j] + r - mp[j][i];
						}
					}
					if(dp[i+1][j] == -1 || dp[i+1][j] > (dp[i][j] + m - mp[j][i])) {
						dp[i+1][j] = dp[i][j] + m - mp[j][i];
					}
				}
			}
