package chap3.completesearch.backtrack.medium.p574;
...
public class Main {
	...
	static int process() throws Exception{
		...
		found = false;
		sb.append(String.format("Sums of %d:\n", t));
		recurse(val, new int[cnt], cnt - 1, 0, t);
		if(!found) {
			sb.append("NONE\n");
		}
		return 1;
	...
	static void recurse(int[][] val, int[] fill, int ind, int curr, int sum) {
		if(curr > sum)
			return;
		if(ind == -1) {
			if(sum != curr)
		...
		for(int i = val[ind][1]; i >= 0; i--) {
			fill[ind] = i;
			recurse(val, fill, ind - 1, curr + (i * val[ind][0]), sum);
		}
	}
}
class InputReader {
