package chap3.completesearch.backtrack.medium.p524;
...
public class Main {
	...
	static int process() throws Exception{
		...
		int[] order = new int[n];
		order[0] = 1;
		recurse(order, 1, 2, prime);
		casen++;
		return 1;
	}
	static void recurse(int[] order, int ind, int sel, boolean[] primes) {

		if(ind == order.length) {
			//System.out.println(Arrays.toString(order));
			if(!primes[order[0] + order[order.length - 1]])
				return;
			String str = "";
		...
		for(int i = 1; i <= order.length; i++) {
			...
			order[ind] = i;
			boolean valid = true;
			if((ind > 0 && primes[order[ind-1] + order[ind]]))
				recurse(order, ind + 1, sel | 1 << i, primes);
			order[ind] = -1;
		}
	}
}
