package chap3.completesearch.backtrack.easy.p10576;
...
public class Main {
	...
	static int process() throws Exception{
		...
		int[] temp = new int[8];
		Arrays.fill(temp, -1);
		recurse(arr, temp, 0, 0);
		System.out.println(String.format("Case %d: %d", casen, min));
		casen++;
		return 1;
	}
	static int min = Integer.MAX_VALUE;
	static void recurse(int[] orig, int[] arr, int ind, int sel) {
		if(ind == arr.length) {
			int cost = 0;
			...
			for(int j = 0; j < 8; j++) {
				...
				for(int i = 0; i < ind; i++) {
					if(Math.abs(ind - i) == Math.abs(arr[ind] - arr[i])) {
						allowed = false;
						break;
					}
				}
				if(allowed)
					recurse(orig, arr, ind + 1, sel | (1 << j));
				arr[ind] = -1;
			}
		}
	}
