package chap3.completesearch.backtrack.hard.p193;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			recurse(al, 0, new ArrayList<Integer>(), new int[n], n);
	...
	static void recurse(List<Integer>[] al, int ind, List<Integer> order, int[] fill, int all) {
		if(ind == al.length) {
			if(order.size() > max) {
				max = order.size();
		...
		if((all + order.size()) <= max)
			return;
		recurse(al, ind + 1, order, fill, all - (fill[ind] > 0 ? 0 : 1));
		if(fill[ind] == 0) {
			order.add(ind);
			int red = 0;
			for(int a : al[ind]) {
			...
			recurse(al, ind + 1, order, fill, all - red - 1);
			for(int a : al[ind]) {
				fill[a]--;
			}
