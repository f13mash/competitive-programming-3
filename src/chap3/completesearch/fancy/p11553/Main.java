package chap3.completesearch.fancy.p11553;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			int[] order = new int[n];
			Arrays.fill(order, -1);
			recurse(arr, 0, order, 0);
			System.out.println(max);
		}
		return 0;
	}
	static int max = Integer.MAX_VALUE;
	static void recurse2(int[][] arr, int[] order1, int[] order, int selected, int ind, int score) {
		if(ind == order.length) {
			...
			for(int i = 0; i < arr[r].length; i++) {
				if(min == arr[r][i] && (selected & (1 << i)) == 0) {
					order[ind] = i;
					recurse2(arr, order1, order, selected | (1 << i), ind + 1, score + arr[r][i]);
					order[ind] = -1;
				}
			}
		}
	}
	static void recurse(int[][] arr, int selected, int[] order, int ind) {
		if(ind == order.length) {
			recurse2(arr, order, new int[order.length], 0, 0, 0);
		}
		else {
			for(int i = 0; i < arr.length; i++) {
				if((selected & (1 << i)) > 0)
					continue;
				order[ind] = i;
				recurse(arr, selected | (1 << i), order, ind + 1);
				order[ind] = -1;
			}
		}
	}
