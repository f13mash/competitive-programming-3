package chap3.completesearch.threeloop.hard.p10660;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
						...
						for(int d = c+1; d < 25; d++) {
							for(int e = d+1; e < 25; e++) {
								...
								for(int i = 0; i < 25; i++) {
									...
									min = Math.min(min, distance(a, i, 5));
									min = Math.min(min, distance(b, i, 5));
									min = Math.min(min, distance(c, i, 5));
									min = Math.min(min, distance(d, i, 5));
									min = Math.min(min, distance(e, i, 5));
									sum += min*mp[i/5][i%5];
								}
								if(sum < res) {
									res = sum;
			...
			System.out.println(String.format("%d %d %d %d %d", arr[0], arr[1], arr[2], arr[3], arr[4]));
		}
		return 0;
	}
	
	static int distance(int a, int b, int base) {
		int i1 = a / base, j1 = a % base;
		int i2 = b / base, j2 = b % base;
