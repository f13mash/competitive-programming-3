package chap3.completesearch.threeloop.hard.p11565;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			boolean found = false;
			for(int x = -(int) Math.ceil(Math.sqrt(c)); x <= Math.sqrt(c) && !found; x++) {
				
				//System.out.println("x : "+x);

				for(int y = Math.max(-(int) Math.sqrt(c - (x*x)), x+1); y <= 10000 && !found; y++) {
					//System.out.println("y : "+y);

					int z = a - x - y;
					if(z <= y)
						continue;
					if((x*z*y) == b && ((x*x) + (y*y) + (z*z)) == c) {
						found = true;
