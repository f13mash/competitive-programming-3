package chap3.greedy.harder.p11157;
...
public class Main {
	...
	static int process() throws Exception{
		...
		while(cases-- > 0) {
			...
			stones[n + 1] = d;
			boolean[] small = new boolean[n+2];
			for(int i = 0; i < n; i++) {
				String[] splits = in.readString().trim().split("-");
				if(splits[0].trim().compareTo("S") == 0) {
					small[i + 1] = true;
				}
				stones[i + 1] = Integer.parseInt(splits[1]);
			}
			...
			while(ind <= n) {
				if(true) {
					if(!small[ind + 1]) {
						max = Math.max(max, stones[ind+1] - stones[ind]);
					}
					else {
						max = Math.max(max, stones[ind+2] - stones[ind]);
					}
					ind++;
				}
				
			}
			System.out.println(String.format("Case %d: %d", casen, max));
			casen++;
		}
		return 0;
	}
