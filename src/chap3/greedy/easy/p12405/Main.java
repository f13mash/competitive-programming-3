package chap3.greedy.easy.p12405;
...
public class Main {
	...
	static int process() throws Exception{
		int n =  in.readInt(), d = in.readInt(), r = in.readInt();
		if(n == 0 && d == 0 && r == 0)
			return 0;
		int[] morn = IOUtils.readIntArray(in, n);
		int[] aft = IOUtils.readIntArray(in, n);
		...
		int extra = 0;
		for(int i = 0; i < n; i++)
			extra += (morn[i] + aft[n - 1 - i] - d) > 0 ? (morn[i] + aft[n - 1 - i] - d)*r : 0;
