package chap3.greedy.sorting.p12210;
...
public class Main {
	...
	static int process() throws Exception{
		...
		if(n == 0 && m == 0)
			return 0;
		int[] bach = IOUtils.readIntArray(in, n);
		int[] spinct = IOUtils.readIntArray(in, m);
		
		Arrays.sort(bach);
		//Arrays.sort(spinct);
		String res = null;
		if(n <= m) {
			res = String.format("Case %d: 0", casen);
		}
		else {
			res = String.format("Case %d: %d %d", casen, n - m, bach[0]);
		}
