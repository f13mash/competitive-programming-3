package chap3.greedy.sorting.p11292;
...
public class Main {
	...
	static int process() throws Exception{
		...
		if(n == 0 && m == 0)
			return 0;
		int[] dragon = IOUtils.readIntArray(in, n);
		int[] height = IOUtils.readIntArray(in, m);
		
		Arrays.sort(dragon);
		Arrays.sort(height);
		String res = "Loowater is doomed!";
		if(n < m) {
			int cost = 0;
			int a = 0, b = 0;
			while(a < n && b < m) {
				if(dragon[a] > height[b]) {
					b++;
				}
				else {
					cost += height[b];
