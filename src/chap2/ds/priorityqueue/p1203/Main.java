package chap2.ds.priorityqueue.p1203;
...
public class Main {
	...
	static int process() throws IOException {
		...
		int k = in.readInt();
		PriorityQueue<Elem> pq = new PriorityQueue<Elem>(1, new Comparator<Elem>() {
		...
		for(Entry<Integer, Integer> val : mp.entrySet()) {
			for(int i = 1; i <= k; i++) {
				pq.add(new Elem(val.getKey(), ((long)val.getValue()) * i));
			}
		}
		for(int i = 0; i < k; i++) {
