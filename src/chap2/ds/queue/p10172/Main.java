...
public class Main {
	...
	static int process() {
		...
		while(cases-- > 0) {
			int n = in.readInt(), s = in.readInt(), q = in.readInt();
			Queue<Integer>[] station = new LinkedList[n];
			for(int i = 0;i < n; i++) {
				station[i] = new LinkedList<>();
				int cnt = in.readInt();
				while(cnt-- > 0) {
					station[i].add(in.readInt());
				}
			}
			
			Stack<Integer> self = new Stack<>();
			int curr = 0;
			int cost = 0;
			while(true) {
				curr %= n;
				Queue<Integer> qu = station[curr];
				int id = curr + 1;
				while(self.size() > 0) {
					if(self.peek() == id) {
						self.pop();
				...
				//System.out.println(curr+" : "+self+" : "+qu+" : "+cost);
				if(done) {
					break;
				}
				cost += 2;
