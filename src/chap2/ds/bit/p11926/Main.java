...
public class Main {
	...
	static int process() {
		...
		int mx = 1000000;
		BitSet bs = new BitSet(mx*2);
		while(n-- > 0) {
			int s = in.readInt();
			int e = in.readInt();
			//System.out.println(n +" : "+ll+" : "+conflict);
			if(conflict) 
				continue;
			conflict = !checkAndInsert(bs, s, e);
		}
		while(m-- > 0) {
			int s = in.readInt();
			int e = in.readInt();
			...
			if(conflict) 
				continue;
			int cnt = 0;
			while((s + (cnt*i)) <= mx && !conflict) {
				conflict = !checkAndInsert(bs, s + (cnt*i), e + (cnt*i));
				cnt++;
			}
		}
		if(conflict) 
	...
	static boolean checkAndInsert(BitSet bs, int s, int e) {
		while(s < e && s <= 1000000) {
			if(bs.get(s)) {
				return false;
			}
