package chap2.ds.treeset.p11136;
...
public class Main {
	...
	static int process() throws IOException {
		...
		while(n-- > 0) {
			...
			Entry<Integer, Integer> first = cnt.firstEntry();
			if(first.getValue() == 1) {
				cnt.remove(first.getKey());
			}
			else {
				cnt.put(first.getKey(), first.getValue() - 1);
			}
			Entry<Integer, Integer> last = cnt.lastEntry();
			if(last.getValue() == 1) {
				cnt.remove(last.getKey());
			}
			else {
				cnt.put(last.getKey(), last.getValue() - 1);
			}
			ans += Math.abs(first.getKey() - last.getKey());
		}
