package chap2.ds.custom.graph.p599;
...
public class Main {
	...
	static int process() throws IOException  {
		...
		while(cases-- > 0) {
			...
			for(int i = 26; i < mp.length; i++) {
				if(mp[i] == -1 && !loop[i-26]) {
					tree++;
				}
			}
		sb.append(String.format("There are %d tree(s) and %d acorn(s).\n", tree, acorn));
		}
