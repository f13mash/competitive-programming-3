package chap2.ds.custom.graph.p10895;
...
public class Main {
	...
	static int process() throws Exception  {
		...
		List<IntegerPair>[] val = new ArrayList[n];
		...
		for(int i = 0; i < m; i++) {
			...
			//System.out.println("cnt : "+cnt+" : : "+Arrays.toString(indx)+" :: "+Arrays.toString(valu));

			for(int j = 0; j < cnt; j++) {
				if(valu[j] == 0)
					continue;
				val[indx[j] - 1].add(new IntegerPair(i + 1, valu[j]));
			}
		}
		StringBuffer sb = new StringBuffer();
		sb.append(String.format("%d %d\n", n, m));
		for(int i = 0; i < n; i++) {
			sb.append(val[i].size());
			for(IntegerPair v : val[i]) {
				sb.append(" "+v.a);
			}
			sb.append("\n");
			boolean f = true;
