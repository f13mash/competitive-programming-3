...
public class Main {
	...
	static int process() throws IOException {
		...
		casen++;
		List<Integer>[] ll = new ArrayList[26];
		for(int i = 0; i < 26; i++) {
			ll[i] = new ArrayList<>();
		}
		...
		for(int i = 25; i >= 0; i--) {
			...
			for(int j = 25; j > i; j--) {
				...
				if(b.size() == 0)
					continue;
				int ind = findClosest(a, b.get(b.size() - 1), false);
				if(ind < 0)
					continue;
				while(a.size() > ind) {
					b.add(a.remove(ind));
		...
		for(int i = 0; i < 26; i++) {
			if(filled[i]){
				//System.out.println((char)('A'+i) + " : "+ll[i]);
				ans++;
			}
		}
		System.out.println(String.format("Case %d: %d", casen, ans));
		return 1;
	}
